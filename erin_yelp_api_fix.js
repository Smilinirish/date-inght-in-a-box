...
function firstApiCall() {
    response = $.ajax({
        ...
        SetCard1Display();
        apiData = firstApiCallData;
        data2 = {
            latitude: apiData.businesses[count1].coordinates.latitude,
            longitude: apiData.businesses[count1].coordinates.longitude,
            categories: $('#catagoryList2 :selected').val(), // you may have to change the categories here and have a second variable called CategoryList2
            radius: $('#distance').val(),
            limit: 25
        };
...
function secondApiCall() {
    ...
    .done(function(response){
            //here we ahve our second api call data
...
function SetCard2Display(){
    //here secondApiCallData should be populated so use that variable for DOM things
    //Need to copy in how you do your Dynamic second card stuff
    
}

function SetCard1Display() {
    var getData = setInterval(() => {
        if (typeof firstApiCallData !== 'undefined') {
            apiData = firstApiCallData;
